...
const STREAM_TEXT = "这是项目页 AI 的流式建议输出，用来验证逐步渲染和推荐区自动收起。";
let session = buildSessionId();

function buildSessionId() {
  return `pstream_${process.pid}_${Date.now().toString(36)}_${Math.floor(Math.random() * 1000)}`;
}
...
async function main() {
  ...
  try {
        ...
        result = extractJsonResult(
          runPw([
            ...
            `async () => {
              ...
              const input = document.getElementById("projectsAssistantInput");
              const form = document.getElementById("projectsAssistantComposer");
              const startersRegion = document.getElementById("projectsAssistantStartersRegion");
              const messages = document.getElementById("projectsAssistantMessages");

              input.value = "请给我一个项目优先级建议。";
    ...
    if (result.assistantMessageCount < 2) {
      throw new Error(`Projects AI should append a new assistant bubble when sending. Got ${result.assistantMessageCount}`);
    }

    if (result.firstStartersState !== "hidden" || result.secondStartersState !== "hidden") {
      throw new Error(
        `Projects AI starter prompts should auto-hide after the first send. Got ${result.firstStartersState} -> ${result.secondStartersState}`,
      );
    }

    if (!result.firstSnapshot) {
      throw new Error("Projects AI should render partial assistant text before the full stream completes.");
    }

    if (result.secondSnapshot.length <= result.firstSnapshot.length) {
      throw new Error(
        `Projects AI should grow the assistant reply during streaming. Got "${result.firstSnapshot}" then "${result.secondSnapshot}"`,
      );
    }

    console.log("PASS: Projects AI streams incremental output and auto-hides starter prompts.");
  } finally {
    server.kill("SIGTERM");
    await wait(300);
