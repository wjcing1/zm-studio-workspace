...
const PAGE_URL = `${BASE_URL}/workspace.html?codex-test-auth=1&workspace-engine=tldraw`;
...
function buildSessionId() {
  return `wwtf_${process.pid}_${Date.now().toString(36)}_${Math.floor(Math.random() * 1000)}`;
}
      ...
      try {
        ...
        runPw([
          ...
          `() => {
            ...
            return fetch("/api/boards/overview", {
              method: "PUT",
              headers: {
        ...
        const result = extractJsonResult(
          runPw([
            ...
            `async () => {
              ...
              if (!viewport || !addFileNodeBtn || !fileInput) {
              ...
              const waitForNode = async () => {
                ...
                while (Date.now() < deadline) {
                  ...
                  const fileNode = board?.nodes?.find((node) => node.type === "file") || null;
                  if (fileNode) {
                    const nodeElement = document.querySelector(\`.workspace-canvas-app [data-workspace-node-id="\${fileNode.id}"]\`);
                    return {
                      fileNode,
                      renderedType: nodeElement?.dataset?.nodeType || null,
                      hasPreviewImage: Boolean(nodeElement?.querySelector("img")),
                      titleInput: nodeElement?.querySelector('[data-file-field="title"]') || null,
              ...
              const titleInput = settled.titleInput;
              if (titleInput instanceof HTMLInputElement) {
                titleInput.focus();
                const descriptor = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value");
                descriptor?.set?.call(titleInput, "Moodboard hero");
                titleInput.dispatchEvent(new Event("input", { bubbles: true }));
                await new Promise((resolve) => setTimeout(resolve, 180));
              }

              const storedBoard = JSON.parse(window.localStorage.getItem("zm-studio-canvas:overview") || "null");
              const storedNode = storedBoard?.nodes?.find((node) => node.id === settled.fileNode.id) || null;

              return {
                ok: true,
                nodeId: settled.fileNode.id,
                storedType: settled.fileNode.type,
                storedTitle: storedNode?.title || "",
                storedFile: storedNode?.file || storedNode?.content || "",
                storedMimeType: storedNode?.mimeType || "",
                storedFileKind: storedNode?.fileKind || "",
                ...
                appSelectedNodeIds: settled.appSelectedNodeIds,
                pageSelectedNodeIds: settled.pageSelectedNodeIds,
              };
            }`,
          ]),
        );
        ...
        if (!result.ok) {
          throw new Error(`Tldraw file node insertion probe failed: ${result.reason || "unknown reason"}`);
        }
        ...
        if (result.storedTitle !== "Moodboard hero") {
          throw new Error(`Tldraw file card title edits should persist. Got ${result.storedTitle}`);
        }
        ...
        if (result.renderedType !== "file") {
          throw new Error(`Inserted file nodes should render as file nodes in tldraw mode. Got ${result.renderedType}`);
        }

        if (!result.hasPreviewImage) {
          throw new Error("Image file nodes should render an inline image preview in tldraw mode.");
        }

        if (!result.hasTitleInput) {
          throw new Error("Tldraw file cards should expose an inline editable title field.");
        }

        if (!result.appSelectedNodeIds.includes(result.nodeId) || !result.pageSelectedNodeIds.includes(result.nodeId)) {
          throw new Error(
            `Uploading a file in tldraw mode should select the created node. App: ${result.appSelectedNodeIds.join(", ")} Page: ${result.pageSelectedNodeIds.join(", ")}`,
          );
        }

        console.log("PASS: workspace tldraw canvas can insert, select, preview, and edit an uploaded image file node.");
        return;
      } catch (error) {
